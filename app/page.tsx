...
export default function Home() {
  ...
  // Keep ref in sync with state so event handlers can read it
  useEffect(() => { rsvpOpenRef.current = rsvpOpen; }, [rsvpOpen]);
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => { e.preventDefault(); if (rsvpOpenRef.current) return; startAudio(); goToSnap(e.deltaY > 0 ? 1 : -1); };
    let touchX = 0;
    const onTouchStart = (e: TouchEvent) => { touchX = e.touches[0].clientX; };
    const onTouchEnd = (e: TouchEvent) => {
      if (rsvpOpenRef.current) return;
      startAudio();
      const dx = touchX - e.changedTouches[0].clientX;
      if (Math.abs(dx) > 20) goToSnap(dx > 0 ? 1 : -1);
    };
  ...
  return (
    <>
      ...
      <div className={s.loader} ref={loaderRef}>
        {/* Couple image with orbiting lotus */}
        <div className={s.loaderCoupleWrap}>
          <div className={s.loaderLotus}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/couple.png" alt="" className={s.loaderLotusImg} />
            <div className={s.loaderRing} /><div className={s.loaderRing2} />
          </div>
          {/* Orbiting lotus */}
          <div className={s.loaderOrbit}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/lotus-small.png" alt="" className={s.loaderOrbitLotus} />
          </div>
        </div>
          ...
          <div className={s.cardInside}>
            ...
            <div className={`${s.ipage} ${s.ipageLight}`} ref={page0Ref}>
              ...
              <div className={s.familyRow}>
                <div className={s.familySide}>
                  <span className={s.covName1}>Sange<br />Dorje</span>
            ...
            <div className={`${s.ipage} ${s.ipageLight}`} ref={page3Ref}>
              ...
              <div className={s.evCard}>
                ...
                <div className={s.evRule} />
                <h2 className={s.evTitle}>Wedding</h2>
                <div className={s.evRule} />
                ...
                <div className={s.evTimeline}>
                  <div className={s.evItem}><span className={s.evTime}>9:40 AM</span><span className={s.evItemLabel}>Bridal Entry</span></div>
                  <div className={s.evItem}><span className={s.evTime}>1:00 PM</span><span className={s.evItemLabel}>Lunch</span></div>
                </div>

                <div className={s.wdVenueDivider} />

                <p className={s.wdVenueLabel}>
                  <a className={s.vnMapLink} href="https://maps.google.com/?q=Dhauladhar+Heights+Resort,+Dharamshala,+Himachal+Pradesh" target="_blank" rel="noopener noreferrer">
                    Dhauladhar Heights Resort ↗
                  </a>
                </p>
                <div className={s.evTimeline}>
                  <div className={s.evItem}><span className={s.evTime}>2:00 PM</span><span className={s.evItemLabel}>Reception</span></div>
                  <div className={s.evItem}><span className={s.evTime}>3:00 PM</span><span className={s.evItemLabel}>Khatak Ceremony</span></div>
                  <div className={s.evItem}><span className={s.evTime}>6:00 PM</span><span className={s.evItemLabel}>Cake Cutting</span></div>
                  <div className={s.evItem}><span className={s.evTime}>7:30 PM</span><span className={s.evItemLabel}>Dinner</span></div>
                  <div className={s.evItem}><span className={s.evTime}>10:00 PM</span><span className={s.evItemLabel}>Farewell</span></div>
                </div>
      ...
      <button className={`${s.rsvpBtn}${rsvpOpen ? ` ${s.rsvpBtnHidden}` : ''}`} onClick={() => {
        setRsvpOpen(true)
      ...
      {/* ── RSVP modal ── */}
      {rsvpOpen && (
        <div className={s.rsvpOverlay} onClick={() => setRsvpOpen(false)}>
          <div className={s.rsvpModal} onClick={e => e.stopPropagation()}>
            <button className={s.rsvpClose} onClick={() => {setRsvpOpen(false);document.body.style.overflow = "auto";}} aria-label="Close">✕</button>

            {rsvpDone ? (
              <div className={s.rsvpThanks}>
                <p className={s.rsvpThanksIcon}>{attending === 'yes' ? '🎉' : '🪷'}</p>
                <h3 className={s.rsvpThanksTitle}>
                  {attending === 'yes' ? 'So glad to hear!' : 'We understand'}
                </h3>
                <p className={s.rsvpThanksSub}>
                  {attending === 'yes'
                    ? `We're overjoyed, ${rsvpName}! Can't wait to celebrate this beautiful day with you.`
                    : `We'll truly miss your presence, ${rsvpName}. Sending you all our love and blessings. 🪷`}
                </p>
              </div>
            ) : (
              <>
                <h3 className={s.rsvpTitle}>Kindly Reply</h3>
                <p className={s.rsvpSub}>Sange Dorje &amp; Kalzang Dolkar</p>
                <div className={s.rsvpRule} />

                <form className={s.rsvpForm} onSubmit={async e => {
                  e.preventDefault();
                  await fetch('/api/rsvp', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ name: rsvpName, phone: rsvpPhone, guests: rsvpCount, attending }),
                  });
                  setRsvpDone(true);
                }}>
                  <label className={s.rsvpLabel}>
                    Your Name
                    <input className={s.rsvpInput} type="text" required placeholder="Full name"
                      value={rsvpName} onChange={e => setRsvpName(e.target.value)} />
                  </label>

                  <label className={s.rsvpLabel}>
                    Phone / WhatsApp
                    <input className={s.rsvpInput} type="tel" required placeholder="+91 XXXXX XXXXX"
                      value={rsvpPhone} onChange={e => setRsvpPhone(e.target.value)} />
                  </label>

                  <label className={s.rsvpLabel}>Number of Guests</label>
                  <div className={s.rsvpCounter}>
                    <button type="button" className={s.rsvpCountBtn}
                      onClick={() => setRsvpCount(c => String(Math.max(1, Number(c) - 1)))}>−</button>
                    <span className={s.rsvpCountNum}>{rsvpCount}</span>
                    <button type="button" className={s.rsvpCountBtn}
                      onClick={() => setRsvpCount(c => String(Math.min(20, Number(c) + 1)))}>+</button>
                  </div>

                  <label className={s.rsvpLabel}>Will you attend?</label>
                  <div className={s.rsvpAttendRow}>
                    <button type="button"
                      className={`${s.rsvpAttendBtn} ${attending === 'yes' ? s.rsvpAttendActive : ''}`}
                      onClick={() => setAttending('yes')}>Joyfully Yes 🎉</button>
                    <button type="button"
                      className={`${s.rsvpAttendBtn} ${attending === 'no' ? s.rsvpAttendActive : ''}`}
                      onClick={() => setAttending('no')}>Regretfully No</button>
                  </div>

                  <button type="submit" className={s.rsvpSubmit} disabled={!attending || !rsvpName || !rsvpPhone}>
                    Send Reply
                  </button>
                </form>
              </>
